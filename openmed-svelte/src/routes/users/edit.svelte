<script context="module">
  export async function load({ fetch, url }) {
    ...
    if (params.has('user')) {
      const response = await fetch(`${url.origin}/api/users/${params.get('user')}`, {
        credentials: 'include'
      });
...
>
  <fieldset class="flex flex-col items-stretch my-3">
    ...
    >
      ...
      {:else}
        <option value={null} disabled="true">Nessun ruolo disponibile</option>
      {/each}
    </select>
    {#if errors?.role}
      <div class="text-red-500">{errors.role[0]}</div>
    {/if}
  </fieldset>
  <fieldset class="flex flex-col items-stretch my-3">
    ...
    <input
      ...
      bind:value={choices.ssn}
      required
    />
    {#if errors?.ssn}
      <div class="text-red-500">{errors.ssn[0]}</div>
    {/if}
  </fieldset>
  ...
  <fieldset class="flex flex-col items-stretch my-3">
    ...
    <input
      ...
      id="surname"
      class="transition-all align-middle appearance-none px-2 py-1 rounded-3xl bg-brandBlue-50/25 hover:bg-brandBlue-50/40"
      bind:value={choices.surname}
      required
    />
    {#if errors?.surname}
      <div class="text-red-500">{errors.surname[0]}</div>
    {/if}
  </fieldset>
  <fieldset class="flex flex-col items-stretch my-4">
    ...
    <input
      ...
      id="address"
      class="transition-all align-middle appearance-none px-2 py-1 rounded-3xl bg-brandBlue-50/25 hover:bg-brandBlue-50/40"
      bind:value={choices.address}
      required
    />
    {#if errors?.address}
      <div class="text-red-500">{errors.address[0]}</div>
    {/if}
  </fieldset>
  <fieldset class="flex flex-col items-stretch my-4">
  ...
  <fieldset class="flex flex-col items-stretch my-4">
    ...
    <input
      ...
      id="confirmPassword"
      class="transition-all align-middle appearance-none px-2 py-1 rounded-3xl bg-brandBlue-50/25 hover:bg-brandBlue-50/40"
      bind:value={choices.confirmPassword}
      required
    />
